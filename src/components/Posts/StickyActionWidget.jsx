...
const StickyActionWidget = ({ diamonds, likes, comments, commentEl }) => {
  ...
  return (
    <div className="sticky-div sticky bottom-2 flex justify-center items-center w-full">
      <div className="action-widget bg-gray-100 border border-slate-800 shadow-xl rounded-full">
        <button className="p-2 rounded-l-full hover:bg-slate-200 duration-300" onClick={diamondHandler}>
          <i className="far fa-gem text-slate-700 px-1"></i>
          {diamonds}
        </button>

        <button className="p-2 border-x border-slate-400 hover:bg-slate-200 duration-300" onClick={likeHandler}>
          <i className="far fa-heart text-slate-700 px-1"></i>
          {likes}
        </button>

        <button className="p-2 rounded-r-full hover:bg-slate-200 duration-300" onClick={commentHandler}>
